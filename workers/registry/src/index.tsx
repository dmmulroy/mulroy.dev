...
app.get("/", (c) => {
  ...
  const page = (
    <Layout
      title="mulroy.dev registry"
      description="shadcn registry for reusable TypeScript utilities"
      registry={registry}
    >
      <div className="max-w-3xl mx-auto px-4 py-16">
        <header className="mb-12">
          ...
          <p className="text-zinc-400">
            shadcn registry for reusable TypeScript utilities
          </p>
        </header>

        <main className="space-y-6">{itemCards}</main>

        <footer className="mt-16 pt-8 border-t border-zinc-800">
          <p className="text-zinc-500 text-sm">
            <a
              href="https://github.com/dmmulroy/mulroy.dev"
...
app.get("/source/:name", async (c) => {
  ...
  const response = await c.env.ASSETS.fetch(
    `https://assets.local/r/${name}.json`,
  );
  ...
  const page = (
    <SourcePage item={item} highlightedCode={highlighted} rawSource={source} />
  );
